...
#else
bool SCD30::begin(I2CArduinoCompat &wirePort, bool autoCalibrate, bool measBegin)
#endif
{
...
bool SCD30::isConnected() {
  ...
  if (_printDebug == true) {
    LOG_DEBUG("Firmware version 0x%04x", fwVer);
  }
...
void SCD30::enableDebugging() { _printDebug = true; }
...
uint16_t SCD30::getAltitudeCompensation(void) { return readRegister(COMMAND_SET_ALTITUDE_COMPENSATION); }

// Set the altitude compenstation. See 1.3.9.
bool SCD30::setAltitudeCompensation(uint16_t altitude) { return sendCommand(COMMAND_SET_ALTITUDE_COMPENSATION, altitude); }
...
// SCD30 soft reset
void SCD30::reset() { sendCommand(COMMAND_RESET); }
...
bool SCD30::beginMeasuring(uint16_t pressureOffset) { return (sendCommand(COMMAND_CONTINUOUS_MEASUREMENT, pressureOffset)); }

// Overload - no pressureOffset
bool SCD30::beginMeasuring(void) { return (beginMeasuring(0)); }

// Stop continuous measurement
bool SCD30::StopMeasurement(void) { return (sendCommand(COMMAND_STOP_MEAS)); }

// Sets interval between measurements
// 2 seconds to 1800 seconds (30 minutes)
bool SCD30::setMeasurementInterval(uint16_t interval) { return sendCommand(COMMAND_SET_MEASUREMENT_INTERVAL, interval); }
...
bool SCD30::readMeasurement(bool skipDataReadyCheck) {
  // Verify we have data from the sensor
  if (skipDataReadyCheck == false && dataAvailable() == false)
    return (false);
  ...
  if (_i2cPort->available()) {
    ...
    for (uint8_t x = 0; x < 18; x++) {
      ...
      default:
        ...
        if (foundCrc != incoming) {
          if (_printDebug == true) {
            LOG_ERROR("readMeasurement: found CRC in byte %d, expected 0x%02x, got 0x%02x", x, foundCrc, incoming);
          }
          error = true;
        }
        break;
    ...
    if (_printDebug == true) {
      LOG_ERROR("readMeasurement: no SCD30 data found from I2C, i2c claims we should receive %d bytes", receivedBytes);
    }
    return false;
  }
  ...
  if (error) {
    if (_printDebug == true)
      LOG_ERROR("readMeasurement: encountered error reading SCD30 data.");
    return false;
  }
  ...
  if (_i2cPort->available()) {
    ...
    *val = (uint16_t)data[0] << 8 | data[1];
    ...
    if (crc == expectedCRC) // Return true if CRC check is OK
      return (true);
    if (_printDebug == true) {
      LOG_ERROR("getSettingValue: CRC fail: expected 0x%02x, got 0x%02x", expectedCRC, crc);
    }
  }
  return (false);
}
