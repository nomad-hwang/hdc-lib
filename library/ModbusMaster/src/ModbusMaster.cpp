...
@ingroup setup
*/
void ModbusMaster::begin(uint8_t slave, UartTransmitter *tx, UartReceiver *rx) {
...
void ModbusMaster::send(uint8_t data) { send(word(data)); }

uint8_t ModbusMaster::available(void) { return _u8ResponseBufferLength - _u8ResponseBufferIndex; }
...
*/
void ModbusMaster::idle(void (*idle)()) { _idle = idle; }
...
*/
void ModbusMaster::preTransmission(void (*preTransmission)()) { _preTransmission = preTransmission; }
...
*/
void ModbusMaster::postTransmission(void (*postTransmission)()) { _postTransmission = postTransmission; }
...
uint8_t ModbusMaster::readHoldingRegisters(uint8_t u8SlaveID, uint16_t u16ReadAddress, uint16_t u16ReadQty) {
  _u8MBSlave = u8SlaveID;
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMasterTransaction(ku8MBReadHoldingRegisters);
}
...
uint8_t ModbusMaster::readInputRegisters(uint8_t u8SlaveID, uint16_t u16ReadAddress, uint8_t u16ReadQty) {
  _u8MBSlave = u8SlaveID;
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModbusMasterTransaction(ku8MBReadInputRegisters);
...
uint8_t ModbusMaster::writeSingleRegister(uint8_t u8SlaveID, uint16_t u16WriteAddress, uint16_t u16WriteValue) {
  _u8MBSlave = u8SlaveID;
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = 0;
  _u16TransmitBuffer[0] = u16WriteValue;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction) {
  ...
  _serial_tx->write(u8ModbusADU, u8ModbusADUSize);
  ...
  u32StartTime = Tick::get();
  while (u8BytesLeft && !u8MBStatus) {

    // TODO
    // _serial_rx->wait(ku16MBResponseTimeout);

    if (_serial_rx->available() > 0) {
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__MODBUSMASTER_DEBUG_PIN_A__, true);
#endif
      u8ModbusADU[u8ModbusADUSize++] = _serial_rx->read();
      u8BytesLeft--;
#if __MODBUSMASTER_DEBUG__
    ...
    if (Tick::elapsed(u32StartTime) > ku16MBResponseTimeout) {
